...
def printPDF_resume_view():
    ...
    #les données
    data = [(get_reshaped_text(u'      الإمضـاء      ' ),get_reshaped_text(u'        بطاقة الهوية      ' ),get_reshaped_text(u'\t\t القيمة (د.ج)\t\t'),get_reshaped_text(u'\tعدد الاسهم\t'),get_reshaped_text(u'\t تاريخ الميلاد \t '),get_reshaped_text(u'          الاســم و اللقــب          '),get_reshaped_text(u'الرقم'))]
    count = 1
    ...
    while i <= nb_pages:
        ...
        for person in donnee :
            temp = (
                "","",person.Solde_part_financiere,person.Solde_finale  , get_reshaped_text(str(person.date_nais)[:10]) , \

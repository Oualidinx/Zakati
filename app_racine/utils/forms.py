...
class MosqueForm(FlaskForm):
    ...
    confirm_password = PasswordField("تأكيـــد كلمــة المرور :", validators=[DataRequired(), EqualTo('password',
                                                                                                     message='كلمتي المرور غير متطابقيتين')])
...
class mosqueForm(FlaskForm):
    ...
    confirm_password = PasswordField("تأكيـــد كلمــة المرور :",
                                     validators=[DataRequired(),
                                                 EqualTo('password', message='كلمتي المرور غير متطابقيتين')])
    submit = SubmitField("إضــافة")
...
class personnes_par_garant(FlaskForm):
    ...
    p_relation_ship = SelectField("صلة القرابــة ", validators=[DataRequired()],
...
class RegisterGarantForm(FlaskForm):
    ...
    situation_sante = SelectMultipleField("الحالــة الصحية :", choices=[(objet.id, objet.id) for objet in
                                                                        Critere.query.filter_by(
                                                                            categorie="الصحية").all()])
    situation_sociale = SelectMultipleField("الحالــة الاجتمـــاعيـة :", choices=[(objet.id, objet.id) for objet in
                                                                                  Critere.query.filter_by(
                                                                                      categorie="الاجتماعية").all()])
    situation_familliale = SelectMultipleField("الحالــة العائليــــــة :", choices=[(objet.id, objet.id) for objet in
                                                                                     Critere.query.filter_by(
                                                                                         categorie="العائلية").all()])
    famille = []

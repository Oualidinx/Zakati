...
class parametre_utils(db.Model):
    ...
    def __repr__(self):
        return '<parametre id={},\ntaux_scolaire={},\ntaux_prime_m={},\nsalaire de base = {}'.format(self.id,
                                                                                                     self.taux_scolaire,
                                                                                                     self.taux_prime_m)
...
class Mosque(db.Model):
    ...
    def tendance(self):  # حساب السهم
        try:
            first = Garant.query.filter_by(mosque_id=self.id).filter_by(actif=1).order_by(
                Garant.Solde_points.asc()).first()
            min_points = first.Solde_points or 1
